...
var MPlayer = (function () {
	function MPlayer(settings,callback) {
		...
		function bindEvents () {
			...
			// 改变音量
			$this.dom.volRange.on($this.settings.volSlideEventName,function (_event,val) {
	...
	$.extend(MPlayer.prototype,{
		 ...
		 * @returns {object}
		 */
		getInfo: function (list,song,_allTime) {
			var $this = this;
			list = list !== undefined ? list : $this.getCurrentList();
			song = song !== undefined ? song : $this.getCurrentSong();
			_alltime=this.getduration();
			return $this.list[list][song][_alltime];
		},
		/**
		 * 通过传入的时间返回指定时间的歌词
